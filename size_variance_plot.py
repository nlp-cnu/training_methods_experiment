...
def main():
    ...
    plt.tight_layout()
    plt.savefig("word_count_variance.pdf", dpi=400)


    # Linear regression for the sample count by variance
    popt, pcov = curve_fit(linear_func, dataset_sample_size, variances)
    sample_residuals = variances - linear_func(dataset_sample_size, popt[0], popt[1])
    ...
    ax.set_xlabel("Number of samples in dataset", fontsize=15)
    ax.set_title("Variance in micro f1 scores", fontsize=15)
    ax.scatter(dataset_sample_size, variances, linewidth=5)
    ax.plot(dataset_sample_size, [linear_func(val, popt[0], popt[1]) for val in dataset_sample_size], label=r"$r^{2}=$" + f"{sample_r_squared:.2f}")

    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles, labels, loc='upper right', fontsize=15)
    plt.tight_layout()
    plt.savefig("sample_count_variance.pdf", dpi=400)
